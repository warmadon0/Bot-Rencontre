...
public class PlayCmd extends MusicCommand
{
    ...
    public PlayCmd(Bot bot, String loadingEmoji)
    {
        ...
        this.help = "joue la chanson fournie";
    ...
    public void doCommand(CommandEvent event) 
    {
        if(event.getArgs().isEmpty() && event.getMessage().getAttachments().isEmpty())
        {
            ...
            if(handler.getPlayer().getPlayingTrack()!=null && handler.getPlayer().isPaused())
            {
                ...
                if(!isDJ)
                    event.replyError("Seuls les DJ peuvent libérer le lecteur!");
                else
                {
                    handler.getPlayer().setPaused(false);
                    event.replySuccess("A repris **"+handler.getPlayer().getPlayingTrack().getInfo().title+"**.");
                }
                return;
            }
            StringBuilder builder = new StringBuilder(event.getClient().getWarning()+" Play Commands:\n");
            builder.append("\n`").append(event.getClient().getPrefix()).append(name).append(" <Titre>` - joue le premier résultat de Youtube");
            builder.append("\n`").append(event.getClient().getPrefix()).append(name).append(" <URL>` - joue la chanson, la playlist ou le flux fourni");
    ...
    private class ResultHandler implements AudioLoadResultHandler
    {
        ...
        private void loadSingle(AudioTrack track, AudioPlaylist playlist)
        {
            ...
            String addMsg = FormatUtil.filter(event.getClient().getSuccess()+" ajouter **"+track.getInfo().title
                    +"** (`"+FormatUtil.formatTime(track.getDuration())+"`) "+(pos==0?"pour commencer à jouer":" à la file d'attente à la position "+pos));
            if(playlist==null || !event.getSelfMember().hasPermission(event.getTextChannel(), Permission.MESSAGE_ADD_REACTION))
                m.editMessage(addMsg).queue();
            else
            {
                new ButtonMenu.Builder()
                        .setText(addMsg+"\n"+event.getClient().getWarning()+" Cette Chanson a une playlist de **"+playlist.getTracks().size()+"** pistes attachées. Sélectionner "+LOAD+" pour charger la playlist.")
                        ...
                        .setAction(re ->
                        {
                            if(re.getName().equals(LOAD))
                                m.editMessage(addMsg+"\n"+event.getClient().getSuccess()+" Charger **"+loadPlaylist(playlist, track)+"** pistes supplémentaires!").queue();
                            else
                                m.editMessage(addMsg).queue();
                        }).setFinalAction(m ->
                        {
